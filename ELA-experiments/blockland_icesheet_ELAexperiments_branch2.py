...
#%%
initialstate2=np.load('ELAiszerofinalstate.npy')
r=6.37e6
ndeg=360.
pi=3.14159265359
...
Hmean=np.mean(Hflat)
Hmax=np.max(Hflat)
icevlist=Hflat*xstep*ystep
ICEvol=np.sum(icevlist)
print('Ice volume is:')
print(ICEvol)
print('Hmean is:')
print(Hmean)
...
plt.contourf(X,Y,Z)#, [levels], **kwargs)
plt.colorbar()
plt.savefig("icesheetcontourELA"+str(ELA)+".png")
...
plt.savefig("icesheet3DHinitELA"+str(ELA)+".png")
...
plt.ylabel('s')
plt.xlabel('Time (yr)')  
plt.savefig("icesheettimeseriesHinitELA"+str(ELA)+".png")


#%%
#np.save('ELAiszerofinalstate.npy', H[:,:,-1])    